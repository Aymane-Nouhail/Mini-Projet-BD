...
'''
def search_records():
	lookup_record_nom_maison = search_entry_nom_maison.get() ; lookup_record_loyer = search_entry_loyer.get()
	...
	records += c.fetchall()
	c.execute("SELECT rowid, * FROM maison WHERE nb_chambres like nb_chambres",
		{
			'nb_chambres':search_entry_nb_chambres.get(),
		})
	records += c.fetchall()
	c.execute("SELECT rowid, * FROM maison WHERE nb_personnes_max like nb_personnes_max",
		{
			'nb_personnes_max':search_entry_nb_personnes_max.get(),
		})
	records += c.fetchall()
	# Add our data to the screen
	global count
...
def lookup_records():
	global search_entry_nom_maison, search_entry_loyer, search_entry_id, search_entry_nb_chambres, search_entry_nb_personnes_max, search_entry_nom_ile, search
	...
	search_entry_id = Entry(search_frame_id, font=("Helvetica", 18))
...
nb_personnes_max_label.grid(row=1, column=0, padx=10, pady=10)
nb_personnes_max_entry = Entry(data_frame)
nb_personnes_max_entry.grid(row=1, column=1, padx=10, pady=10)

nom_ile_label = Label(data_frame, text="Nom ile")
nom_ile_label.grid(row=1, column=2, padx=10, pady=10)
nom_ile_entry = Entry(data_frame)
nom_ile_entry.grid(row=1, column=3, padx=10, pady=10)


nb_chambres_label = Label(data_frame, text="nombre chambres")
nb_chambres_label.grid(row=1, column=4, padx=10, pady=10)
nb_chambres_entry = Entry(data_frame)
nb_chambres_entry.grid(row=1, column=5, padx=10, pady=10)
...
def select_record(e):
	...
	nb_personnes_max_entry.insert(0, values[3])
	nom_ile_entry.insert(0, values[4])
...
def update_record():
	...
	c.execute("""UPDATE maison SET
		nom_maison = :first,
		Loyer = :last,
		nb_chambres = :nb_personnes_max,
		nb_personnes_max = :nom_ile,
		nom_ile = :state,

		WHERE oid = :oid""",
		{
			'first': nom_maison_entry.get(),
			'last': loyer_maison_entry.get(),
