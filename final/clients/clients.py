...
def search_records():
	lookup_record_prenom = search_entry_prenom.get()
	lookup_record_nom = search_entry_nom.get()
	...
	c = conn.cursor()
	records = []
	if lookup_record_prenom != '':
		c.execute("SELECT rowid, * FROM clients WHERE prenom like :prenom",
			{
				'prenom':lookup_record_prenom,
			})
		records = c.fetchall()
	if lookup_record_nom != '':
		c.execute("SELECT rowid, * FROM clients WHERE nom like :nom",
			{
				'nom':lookup_record_nom,
			})
		records = c.fetchall()
	# Add our data to the screen
	global count
...
def lookup_records():
	global search_entry_prenom, search,search_entry_nom
	...
	# Create label frame
	search_frame_prenom = LabelFrame(search, text="prenom")
	search_frame_prenom.pack(padx=10, pady=10)
	
	search_frame_nom = LabelFrame(search, text="nom")
	search_frame_nom.pack(padx=10, pady=10)

	# Add entry box
	search_entry_prenom = Entry(search_frame_prenom, font=("Helvetica", 18))
	search_entry_prenom.pack(pady=20, padx=20)
	
	search_entry_nom = Entry(search_frame_nom, font=("Helvetica",18))
	search_entry_nom.pack(pady=20, padx=20)
